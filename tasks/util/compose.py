...
ENV_VARS = {
    "FAABRIC_EXP_IMAGE_NAME": "{}/{}:{}".format(
        ACR_NAME, FAABRIC_EXP_IMAGE_NAME, get_version()
    ),
...
def run_compose_cmd(compose_cmd, capture_output=False):
    ...
    # by name
    docker_cmd = "docker compose -f {} {}".format(
        OPENMMPI_COMPOSE_FILE, compose_cmd
    )
...
def get_compose_ctrs():
    ...
    docker_ip_cmd = "docker inspect -f"
    docker_ip_cmd += " '{{{{range.NetworkSettings.Networks}}}}{{{{.IPAddress"
    docker_ip_cmd += "}}}}{{{{end}}}}' {}"
